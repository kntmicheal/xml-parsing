...
$files = glob('/DSD/Reports/Shreds/*.{xml, XML}', GLOB_BRACE);
...
foreach($files as $file) {
  ...
  */
  ...
  $drive->method          = (string)$xml_file->result->disk->method;
  $drive->method_passes   = (int)$xml_file->result->disk->passes;
  $drive->status          = (string)$xml_file->result->disk->status;
  $drive->operator        = (string)$xml_file->result->disk->operator;
  $drive->started         = (string)$xml_file->result->disk->started;
  $drive->elapsed         = (string)$xml_file->result->disk->elapsed;
  $drive->verified        = ((string)$xml_file->attributes->verification->attributes()->use == "Yes") ? 1 : 0 ;
  $drive->verified_passes = (int)$xml_file->attributes->verification->attributes()->passes;
  ...
  */
  ...
  $service_url = 'http://10.5.45.180/dev/dsd/api/shreds';
  ...
  */
  ...
  if($httpcode != 200) {
    ...
    $moved = rename($file, '/DSD/Reports/Shreds/'. $filename);
    if($moved) {
      echo "Moved the file back to the original location! \n";
    }
  } else {
    echo "Success! HTTP response code is:  " . $httpcode . "\n";
    $moved = rename($file, '/DSD/Archive/Reports/Shreds/' . $filename);
    if($moved) {
